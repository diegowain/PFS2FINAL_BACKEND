...
export default class CandidatoDAO{
    ...
    async incluir(candidato){
        ...
        if (candidato instanceof Candidato){
            ...
            const sql="INSERT INTO candidato(cpf,nome,endereco, \
                                           telefone, ) \
                                           VALUES(?,?,?,?)";
    ...
    async alterar(candidato){
        ...
        if (candidato instanceof Candidato){
            const conexao = await conectar();
            const sql="UPDATE candidato SET nome=?, endereco = ?,telefone = ? \
                       WHERE cpf=?";
            const valores = [candidato.nome,candidato.endereco, candidato.telefone,candidato.cpf];                                        
            await conexao.query(sql,valores);
            global.poolConexoes.releaseConnection(conexao);
        }
    }
    ...
    async consultar(termo){
        ...
        for(const row of rows){
            const candidato = new Candidato(row['cpf'],row['nome'],
            row['endereco'], row['telefone']);
